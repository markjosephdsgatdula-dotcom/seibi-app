      ...
      try {
        ...
        const templatesSeed = [
            { id: 'template-co2-mag', name: 'CO2/MAG Robot Template', items: [
              { id: 1, title: '溶接機裏側のフィルター清掃', title_en: 'Clean Welder Rear Filter', title_jp: '溶接機裏側のフィルター清掃', desc: 'パワーソース（溶接電源）裏側のフィルターを取り外し、粉塵をエアーブローするか、新しいものに交換する。', desc_en: 'Remove rear filter and blow out dust with compressed air or replace with new one.', desc_jp: 'パワーソース（溶接電源）裏側のフィルターを取り外し、粉塵をエアーブローするか、新しいものに交換する。', freq: 'monthly', image: 'image1.jpeg', category: 'A' },
              { id: 2, title: '定盤位置出し・各軸合わせマークの確認', title_en: 'Verify Robot & Joint Alignment', title_jp: '定盤位置出し・各軸合わせマークの確認', desc: '基準校正プログラムを実行し、定盤A、B、Cでの位置確認と同時に、J1〜J6各軸の合わせマーク（矢印・ケガキ線）が一致しているか目視確認する。', desc_en: 'Run calibration program (A, B, C) and verify J1-J6 alignment marks visually.', desc_jp: '基準校正プログラムを実行し、定盤A、B、Cでの位置確認と同時に、J1〜J6各軸の合わせマーク（矢印・ケガキ線）が一致しているか目視確認する。', freq: 'monthly', image: 'image10.jpeg', category: 'A' },
              { id: 3, title: '非常停止ボタンの作動確認', title_en: 'Test Emergency Stop Button', title_jp: '非常停止ボタンの作動確認', desc: 'すべての非常停止ボタンがロボットを即座に停止させるか確認する。', desc_en: 'Ensure all Emergency buttons halt the robot immediately.', desc_jp: 'すべての非常停止ボタンがロボットを即座に停止させるか確認する。', freq: 'monthly', image: 'image4.jpeg', category: 'A' },
              { id: 4, title: '溶接アース接続部の緩み点検', title_en: 'Check Welding Ground Cable', title_jp: '溶接アース接続部の緩み点検', desc: 'アースクランプおよび定盤の接続部に緩みがないか点検する。', desc_en: 'Inspect grounding clamps and table connections for looseness.', desc_jp: 'アースクランプおよび定盤の接続部に緩みがないか点検する。', freq: 'monthly', image: 'generic-check.png', category: 'A' },
              { id: 5, title: 'トーチ衝突検知センサー動作テスト', title_en: 'Test Torch Shock Sensor', title_jp: 'トーチ衝突検知センサー動作テスト', desc: 'トーチの衝突検知センサーを手動で軽くたわませ、安全回路が作動してロボットが即座に一時停止することを確認する。', desc_en: 'Manually deflect the torch to ensure the collision safety circuit halts the robot.', desc_jp: 'トーチの衝突検知センサーを手動で軽くたわませ、安全回路が作動してロボットが即座に一時停止することを確認する。', freq: 'monthly', image: 'generic-check.png', category: 'A' },
              { id: 6, title: '送給装置の送給圧調整（3.5に調整）', title_en: 'Verify Wire Feed Pressure (3.5)', title_jp: '送給装置の送給圧調整（3.5に調整）', desc: '送給装置の加圧ローラーの圧力設定を確認し、ワイヤーの滑りや潰れを防止するために「3.5」に調整・維持する。', desc_en: 'Check and adjust wire feed roller pressure to exactly 3.5 to prevent slipping.', desc_jp: '送給装置の加圧ローラーの圧力設定を確認し、ワイヤーの滑りや潰れを防止するために「3.5」に調整・維持する。', freq: 'monthly', image: 'generic-check.png', category: 'A' },
              { id: 7, title: '定盤・溶接台の清掃（研磨・防錆油塗布）', title_en: 'Sand & Oil Welding Table', title_jp: '定盤・溶接台の清掃（研磨・防錆油塗布）', desc: 'バフ研磨機でスパッタを除去し、溶接定盤に防錆油を塗布する。', desc_en: 'Remove spatter with a buffer and apply anti-rust oil to the welding table.', desc_jp: 'バフ研磨機でスパッタを除去し、溶接定盤に防錆油を塗布する。', freq: 'monthly', image: 'generic-check.png', category: 'A' },
              { id: 8, title: 'コンジットホース内のアルコール清掃', title_en: 'Clean Conduit Hose Inner Liner', title_jp: 'コンジットホース内のアルコール清掃', desc: 'コンジットホースをアルコールで清掃する。', desc_en: 'Clean the conduit hose with alcohol.', desc_jp: 'コンジットホースをアルコールで清掃する。', freq: 'semi-annual', image: 'image11.jpeg', category: 'A' },
              { id: 9, title: '溶接機・基板内部のエアブロー清掃', title_en: 'Blow Air Inside Welder & Boards', title_jp: '溶接機・基板内部のエアブロー清掃', desc: 'カバーを取り外し、内部基板やパワー半導体に付着した導電性粉塵を乾燥したエアーで吹き飛ばす。', desc_en: 'Remove the cover and blow dry air to remove conductive dust from PCBs and power modules.', desc_jp: 'カバーを取り外し、内部基板やパワー半導体に付着した導電性粉塵を乾燥したエアーで吹き飛ばす。', freq: 'annual', image: 'image12.jpeg', category: 'A' },
              { id: 10, title: 'ロボット台座・取付ボルト緩みの目視点検', title_en: 'Visual Check of Mounting Bolts', title_jp: 'ロボット台座・取付ボルト緩みの目視点検', desc: 'ロボットベース固定用アンカーボルトが緩んでいないか、合いマークのズレや目視確認、または打音テストによって確認する。', desc_en: 'Visually check base anchor bolts and alignment marks, or perform tap testing.', desc_jp: 'ロボットベース固定用アンカーボルトが緩んでいないか、合いマークのズレや目視確認、または打音テストによって確認する。', freq: 'annual', image: 'generic-check.png', category: 'A' }
            ]},
            { id: 'template-regulator', name: 'Gas Regulator Template', items: [
              { id: 1, title: 'ガス漏れ確認', title_en: 'Check Gas Leak', title_jp: 'ガス漏れ確認', desc: 'ガス漏れ確認（レギュレーター、カプラ、配管接続部に探知スプレー）', desc_en: 'Check for gas leaks (apply leak detector spray to the regulator, coupler, and pipe connections).', desc_jp: 'ガス漏れ確認（レギュレーター、カプラ、配管接続部に探知スプレー）', freq: 'monthly', image: 'image8.jpeg' },
              { id: 2, title: '流量12L/min調整', title_en: 'Adjust & Verify Flow Rate', title_jp: '流量12L/min調整', desc: '流量12L/min調整・動作確認', desc_en: 'Adjust the flow rate to 12L/min and verify proper operation.', desc_jp: '流量12L/min調整・動作確認', freq: 'monthly', image: 'image8.jpeg' }
            ]},
            { id: 'template-utility-gas', name: 'Main Gas Utility Template', items: [
              { id: 1, title: 'ガス圧計ゲージの針確認', title_en: 'Check Gas Pressure Needle', title_jp: 'ガス圧計ゲージの針確認', desc: '1F中庭 ガス圧計ゲージの針確認（異常時は大丸エナウィンに連絡）', desc_en: 'Check the gas pressure gauge needle in the 1F Courtyard (contact Daimaru Enawin if abnormal).', desc_jp: '1F中庭 ガス圧計ゲージの針確認（異常時は大丸エナウィンに連絡）', freq: 'monthly', image: 'images/gas-utility.jpg' },
              { id: 2, title: 'ガス漏れ確認', title_en: 'Check Gas Leak', title_jp: 'ガス漏れ確認', desc: 'ガス漏れ確認（レギュレーター、カプラ、配管接続部に探知スプレー）', desc_en: 'Check for gas leaks (apply leak detector spray to the regulator, coupler, and pipe connections).', desc_jp: 'ガス漏れ確認（レギュレーター、カプラ、配管接続部に探知スプレー）', freq: 'monthly', image: 'images/gas-leak.jpg' }
            ]},
            { id: 'template-grinder', name: 'Grinder & Sander Template', items: [
              { id: 1, title: '電気配線の破損確認', title_en: 'Inspect Power Cable', title_jp: '電気配線の破損確認', desc: '電気配線の破損確認（目視による被覆の亀裂や断線の有無）', desc_en: 'Visually inspect the power cable for any damage (look for cracks in the insulation or broken wires).', desc_jp: '電気配線の破損確認（目視による被覆の亀裂や断線の有無）', freq: 'monthly', image: 'generic-check.png' },
              { id: 2, title: '研磨ベルトの摩耗確認', title_en: 'Check Grinding Belt Wear', title_jp: '研磨ベルトの摩耗確認', desc: '研磨ベルトの摩耗、ひび割れ、目詰まりの確認', desc_en: 'Check the grinding belt for wear, cracks, or clogging.', desc_jp: '研磨ベルトの摩耗、ひび割れ、目詰まりの確認', freq: 'monthly', image: 'generic-check.png' },
              { id: 3, title: '異音・異常振動確認', title_en: 'Test Abnormal Vibration / Sound', title_jp: '異音・異常振動確認', desc: '無負荷状態で運転させ、スイッチの作動、異音・異常振動が無いか確認', desc_en: 'Run the tool without load to verify switch operation, and check for abnormal noises or vibrations.', desc_jp: '無負荷状態で運転させ、スイッチの作動、異音・異常振動が無いか確認', freq: 'monthly', image: 'generic-check.png' }
            ]}
          ];

        if (data) {
          let list = (Array.isArray(data) ? data : Object.values(data)).filter(t => t && t.id);
          let needsMigration = false;
          
          const migratedList = list.map(tpl => {
            const seedTpl = templatesSeed.find(s => s.id === tpl.id);
            if (seedTpl) {
              const needsMigrationForTpl = tpl.items && tpl.items.some(item => {
                if (!item) return false;
                const seedItem = seedTpl.items.find(si => si && si.id === item.id);
                if (!seedItem) return false;
                const seedTitleHasJp = /[\u3040-\u30ff\u4e00-\u9fff]/.test(seedItem.title);
                const itemTitleHasJp = item.title && /[\u3040-\u30ff\u4e00-\u9fff]/.test(item.title);
                const isTitleMissingJp = seedTitleHasJp && !itemTitleHasJp;
                const isTitleJpMissing = !item.title_jp;
                return isTitleMissingJp || isTitleJpMissing;
              });
              if (needsMigrationForTpl) {
                needsMigration = true;
                console.log(`[FirebaseSync] Migrating template ${tpl.id} to include translations & Japanese defaults.`);
                const mergedItems = tpl.items.map(item => {
                  ...
                  if (seedItem) {
                    const seedTitleHasJp = /[\u3040-\u30ff\u4e00-\u9fff]/.test(seedItem.title);
                    const itemTitleHasJp = item.title && /[\u3040-\u30ff\u4e00-\u9fff]/.test(item.title);
                    const shouldOverwriteTitle = seedTitleHasJp && !itemTitleHasJp;
                    const shouldOverwriteJp = !item.title_jp;
                    return {
                      ...item,
                      title: shouldOverwriteTitle ? seedItem.title : item.title,
                      desc: shouldOverwriteTitle ? seedItem.desc : item.desc,
        ...
        console.error('[FirebaseSync] Error in templates sync callback:', err);
      } finally {
    ...
    db.ref('notices').on('value', snapshot => {
      ...
      if (data) {
        ...
        if (isLegacyArray) {
          ...
          console.log('[FirebaseSync] Migrating /notices from array format to ID-keyed format...', data);
          const migrated = {};
          Object.values(data).forEach((n, idx) => {
            if (n) {
              const id = n.id || `notice-${Date.now()}_${idx}`;
              n.id = id;
              migrated[id] = n;
            }
          });
          console.log('[FirebaseSync] Writing migrated notices to Firebase:', migrated);
          db.ref('notices').set(migrated)
            .then(() => console.log('[FirebaseSync] Notices migration write succeeded.'))
            .catch(err => console.error('[FirebaseSync] Notices migration write failed:', err));
